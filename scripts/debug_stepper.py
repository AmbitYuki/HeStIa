...
def run(env, policy):
    ...
    viewer.add_line_to_fig(line_name="speed-y", fig_idx=0)
    viewer.add_line_to_fig(line_name="speed-z", fig_idx=0)
    done = False
    ts, end_ts = 0, 2000
    ep_rewards = []
    ...
    while (ts < end_ts) and (done == False):
        ...
        # camare follow robot
        viewer.cam.lookat[0] = env.task._client.get_object_xpos_by_name(env.task._root_body_name, 'OBJ_BODY')[0] + 1
        viewer.cam.lookat[1] = env.task._client.get_object_xpos_by_name(env.task._root_body_name, 'OBJ_BODY')[1]
        vel = env.task._client.get_body_vel("PELVIS_S")[0]
        viewer.add_data_to_line(line_name="speed-x", line_data=vel[0], fig_idx=0)
        viewer.add_data_to_line(line_name="speed-y", line_data=vel[1], fig_idx=0)
        viewer.add_data_to_line(line_name="speed-z", line_data=vel[2], fig_idx=0)
