...
# python scripts/debug_stepper.py --path D:\\studyspace\\LearningHumanoidWalking\\trained\\walk_with_arm_perform_better_04015\\
# python scripts/debug_stepper.py --path D:\\studyspace\\LearningHumanoidWalking_waist_finished\\walk_with_arm_perform_better_04015\\
...
def run_experiment(args):
    ...
    if args.continued:
        ...
        policy = Gaussian_FF_Actor(obs_dim, action_dim, layers=(512, 256), fixed_std=np.exp(args.std_dev), bounded=False)
        critic = FF_V(obs_dim, layers=(512, 256))
        with torch.no_grad():
